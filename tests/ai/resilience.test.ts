...
function mockFetch(fn: (url: string, init: any, call: number) => Promise<Response>) {
  ...
  return vi.spyOn(globalThis, "fetch").mockImplementation(((url: any, init: any) =>
    fn(String(url), init, ++call)) as any);
}
...
describe("resilientFetch — timeout", () => {
  it("aborta e lança erro amigável ao estourar o tempo limite", async () => {
    ...
    await expect(
      resilientFetch("http://x", {}, { timeoutMs: 40, retries: 0 })
...
describe("chatComplete — erro não vaza o corpo do provedor (FR-010)", () => {
  it("500 com corpo → mensagem genérica, sem o corpo", async () => {
    mockFetch(async () => new Response(JSON.stringify({ error: { message: "SEGREDO-INTERNO-DO-PROVEDOR" } }), { status: 500 }));
    await expect(
    ...
    await chatComplete({ settings, system: "s", messages: [{ role: "user", content: "oi" }] }).catch((e) => {
